...
if (isInDeleteMode) {
    ...
    db.execute(`INSERT INTO books(title, description, number, unreleased, img, series_id) VALUE (?, ?, ?, ?, ?, ?);`, ["All Systems Red", "An award-winning sci-fi bestseller about the humanity of Artificial Intelligence.", 1, false, "https://upload.wikimedia.org/wikipedia/en/8/8f/All_Systems_Red_-_The_Murderbot_Diaries_1_%28cover%29.jpg", 5])
    db.execute(`INSERT INTO books(title, description, number, unreleased, img, series_id) VALUE (?, ?, ?, ?, ?, ?);`, ["Artificial Condition", "An award-winning sci-fi bestseller about the humanity of Artificial Intelligence.", 2, false, "https://media.forbiddenplanet.com/products/e0/91/5066da797b3f7d17c307ba7847550af54b2e.jpg", 5])
    db.execute(`INSERT INTO books(title, description, number, unreleased, img, series_id) VALUE (?, ?, ?, ?, ?, ?);`, ["Rogue Protocol", "An award-winning sci-fi bestseller about the humanity of Artificial Intelligence.", 3, false, "https://upload.wikimedia.org/wikipedia/en/8/8f/All_Systems_Red_-_The_Murderbot_Diaries_1_%28cover%29.jpg", 5])
    db.execute(`INSERT INTO books(title, description, number, unreleased, img, series_id) VALUE (?, ?, ?, ?, ?, ?);`, ["Exit Strategy", "An award-winning sci-fi bestseller about the humanity of Artificial Intelligence.", 4, false, "https://m.media-amazon.com/images/I/51bndThl8tL.jpg", 5])
    db.execute(`INSERT INTO books(title, description, number, unreleased, img, series_id) VALUE (?, ?, ?, ?, ?, ?);`, ["Network Effect", "An award-winning sci-fi bestseller about the humanity of Artificial Intelligence.", 5, false, "https://kbimages1-a.akamaihd.net/69f046c9-cd75-47e1-971f-f926f5ff6611/1200/1200/False/network-effect-1.jpg", 5])
    db.execute(`INSERT INTO books(title, description, number, unreleased, img, series_id) VALUE (?, ?, ?, ?, ?, ?);`, ["Fugitive Telemetry", "An award-winning sci-fi bestseller about the humanity of Artificial Intelligence.", 6, false, "https://m.media-amazon.com/images/I/41fAH7017RL._AC_SY1000_.jpg", 5])
    db.execute(`INSERT INTO books(title, description, number, unreleased, img, series_id) VALUE (?, ?, ?, ?, ?, ?);`, ["A Court of Thorns and Roses", "Feyre is a huntress. She thinks nothing of slaughtering a wolf to capture its prey. But, like all mortals, she fears what lingers mercilessly beyond the forest. And she will learn that taking the life of a magical creature comes at a high price…", 1, false, "https://kbimages1-a.akamaihd.net/b3fcbbc4-ccab-4304-a971-ce95f4a0de7e/1200/1200/False/a-court-of-thorns-and-roses-1.jpg", 3])
    db.execute(`INSERT INTO books(title, description, number, unreleased, img, series_id) VALUE (?, ?, ?, ?, ?, ?);`, ["A Court of Mist and Fury", "Feyre is a huntress. She thinks nothing of slaughtering a wolf to capture its prey. But, like all mortals, she fears what lingers mercilessly beyond the forest. And she will learn that taking the life of a magical creature comes at a high price…", 2, false, "https://images.booksense.com/images/583/575/9781635575583.jpg", 3])
